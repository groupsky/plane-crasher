...
class Bots extends Phaser.Stage {
  create () {
    ...
    const botsTitle = new Label(this.game, panelLeft.x + panelLeft.width * 0.5, panelLeft.y + 6, 'Bots', styles.titleStyle)
    botsTitle.anchor.set(0.5, 0)
    const botsTotalLabel = new Label(this.game, panelLeft.x + panelLeft.width - 8, panelLeft.y + panelLeft.height - 8,
      'Bots: ' + Math.floor(idle.bots.items.length), styles.statStyle)
    botsTotalLabel.anchor.set(1, 1)
    const botsUpgradeHint = new Label(this.game, panelLeft.x + 8, panelLeft.y + panelLeft.height - 8,
